...
//  ex7
//
//  Created by cseb65 on 27/09/21.
//  Copyright © 2021 cseb65. All rights reserved.
//
...
void cohen_line(float x1,float y1,float x2,float y2)
{
    ...
    while((c1|c2)>0)
    {
        ...
        if((c & 8)>0)
        {
            y=ymax;//horizontal top intersection
            x=xi+1.0/m*(ymax-yi);
        }
        else
            if((c & 4)>0)
            {
                y=ymin;//horizontal bottom intersection
                x=xi+1.0/m*(ymin-yi);
            }
        else
          if((c & 2)>0)
           {
               x=xmax;//vertical right intersection
               y=yi+m*(xmax-xi);
           }
        else
           if((c & 1)>0)
          {
             x=xmin;//vertical left intersection
             y=yi+m*(xmin-xi);
          }
        if(c==c1)
        {
